# -*- coding: utf-8 -*-
"""
Created on Sun Dec 27 00:57:25 2022
...
for id in tqdm(range(49,64)):  
    ...
    elastic = ElasticNetCV(l1_ratio=np.arange(0.1, 1.1, 0.1), tol=0.01, eps=0.001, cv=10, max_iter = n_est)
    elastic.fit(X, y)
    print("\nBest alpha value: ", elastic.alpha_)
    print("\nL1 ratio: ", elastic.l1_ratio_)
    Elastic = ElasticNet(alpha=elastic.alpha_, l1_ratio = elastic.l1_ratio_, max_iter=n_est)
    ...
    importances=np.abs(Elastic.coef_)
    ...
    feat_sel_data.to_csv(r'/home/UFAD/shajib.ghosh/CompStat_Feature_Selection/Supervised/Embedded/Results/ElasticNet/'+'ElasticNet_K'+str(k)+'_'+id+'.csv', index = None)
...
df_results.to_csv(r'/home/UFAD/shajib.ghosh/CompStat_Feature_Selection/Supervised/Embedded/Results/'+ 'ElasticNet_Regression_summary.csv')
print("Process completed for Feature Selection using ElasticNet Regression.")